...
const ITEMS = [{name: 'Frida'}, {name: 'Cofia'}, {name: 'Van Gogh'}];
const FILTERED_ITEMS = [{name: 'Frida'}];
...
test('should initially load all listings', function (assert) {
    ...
    return wait().then(() => {
        ...
        assert.equal(this.$('.name').first().text().trim(), 'Frida');
    });
});
...
test('should update with matching listings', function (assert) {
    ...
    this.$('.list-filter input').val('Fri').keyup();

    return wait().then(() => {
        assert.equal(this.$('.name').length, 1);
        assert.equal(this.$('.name').text().trim(), 'Frida');
    });
});